...
export function LocationSelector({
  ...
  return (
    <div className={cn(wrapper, className)}>
      ...
      <div className="space-y-1.5">
        <Label className="text-sm font-medium">
          State / Province{required && showState && <span className="text-destructive ml-0.5">*</span>}
        </Label>
        <Select
          value={state}
          onValueChange={v => { onStateChange(v); onCityChange(""); }}
          disabled={!country || !showState}>
          <SelectTrigger className={cn("h-9", errors?.state && "border-destructive", (!country || !showState) && "opacity-50")}>
