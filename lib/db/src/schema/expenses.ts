...
export const expenseApprovalLogsTable = pgTable("expense_approval_logs", {
  ...
  action:     text("action").notNull(),  // submitted | approved | rejected | forwarded | reimbursed | paid
...
export const gstDocumentsTable = pgTable("gst_documents", {
  id:             serial("id").primaryKey(),
  docType:        text("doc_type").notNull(),        // invoice | bill | credit_note | debit_note | receipt | e_way_bill
  docNo:          text("doc_no").notNull(),
  docDate:        date("doc_date", { mode: "string" }).notNull(),
  // Party
  partyName:      text("party_name").notNull(),
  partyGstin:     text("party_gstin"),
  partyType:      text("party_type").notNull().default("supplier"), // customer | supplier
  ...
  gstRate:        numeric("gst_rate", { precision: 5, scale: 2 }),
  // Filing
  period:         text("period"),                   // "2025-06"
  filingStatus:   text("filing_status").notNull().default("unfiled"), // unfiled | filed | matched | mismatched | cancelled
  // Linked
